...
PACKAGENAME="github.com/helviojunior/adbcat"
...
get-version:
	...
	if [ "$(VER)" = "dev" ]; then \
		if [ -f ${TARGET}/.version ]; then \
			...
			curl -s https://api.github.com/repos/helviojunior/adbcat/releases/latest \
...
windows: get-version	## Make Windows x86 and x64 Binaries
	@for ARCH in ${ARCHS}; do \
		...
		GOOS=windows GOARCH=$${ARCH} go build -a -ldflags "$$(cat ${TARGET}/.ldflags)" -o ${TARGET}/adbcat_windows_$${ARCH}.exe || exit 1 ;\
	done; \
	echo "Done."
...
linux: get-version	## Make Linux x86 and x64 Binaries
	@for ARCH in ${ARCHS}; do \
		...
		GOOS=linux GOARCH=$${ARCH} go build -a -ldflags "$$(cat ${TARGET}/.ldflags)" -o ${TARGET}/adbcat_linux_$${ARCH} || exit 1 ;\
	done; \
	echo "Done."
...
mac: get-version	## Make Darwin (Mac) x86 and x64 Binaries
	@for ARCH in ${ARCHS}; do \
		if [ "$${ARCH}" != "386" ]; then \
			...
			GOOS=darwin GOARCH=$${ARCH} go build -a -ldflags "$$(cat ${TARGET}/.ldflags)" -o ${TARGET}/adbcat_darwin_$${ARCH} || exit 1 ;\
		fi;\
	done; \
	echo "Done."
