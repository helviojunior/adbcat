...
url=$(curl -s https://api.github.com/repos/helviojunior/adbcat/releases | jq -r '[ .[] | {id: .id, tag_name: .tag_name, assets: [ .assets[] | select(.name|match("linux-amd64.tar.gz$")) | {name: .name, browser_download_url: .browser_download_url} ]} | select(.assets != []) ] | sort_by(.id) | reverse | first(.[].assets[]) | .browser_download_url')

cd /tmp
rm -rf adbcat-latest.tar.gz adbcat
wget -nv -O adbcat-latest.tar.gz "$url"
tar -xzf adbcat-latest.tar.gz

rsync -av adbcat /usr/local/sbin/
...
url=$(curl -s https://api.github.com/repos/helviojunior/adbcat/releases | jq -r --arg filename "darwin-${arch}.tar.gz\$" '[ .[] | {id: .id, tag_name: .tag_name, assets: [ .assets[] | select(.name|match($filename)) | {name: .name, browser_download_url: .browser_download_url} ]} | select(.assets != []) ] | sort_by(.id) | reverse | first(.[].assets[]) | .browser_download_url')

cd /tmp
rm -rf adbcat-latest.tar.gz adbcat
curl -sS -L -o adbcat-latest.tar.gz "$url"
tar -xzf adbcat-latest.tar.gz
...
```
# Download latest helviojunior/adbcat release from github
function Invoke-Downloadadbcat {

    $repo = "helviojunior/adbcat"
    ...
    Write-Host "Getting adbcat version banner"
    . $name version 
}
...
Function Get-AssetData {
    ...
    if (Get-Member -inputobject $Release -name "assets" -Membertype Properties) {
        ...
        {
            If ($asset.name.Contains("adbcat-") -and $asset.name.Contains("$platform-$arch$extension")) { Return $asset }
        }
...
```
git clone https://github.com/helviojunior/adbcat.git
cd adbcat
go get ./...
go build
```
...
```
go install .
ln -s /root/go/bin/adbcat /usr/bin/adbcat
```
